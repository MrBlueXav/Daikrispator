...
void place_slope_dd(float *buffer, int index, float phase, float w,
		float slope_delta) {
	float r;
	int i;
	...
	while (i < MINBLEP_PHASES * SLOPE_DD_PULSE_LENGTH) {
		buffer[index] += slope_delta
				* (slope_dd_table[i]
						+ r * (slope_dd_table[i + 1] - slope_dd_table[i]));
		i += MINBLEP_PHASES;
		index++;
	}
}
...
float VCO_bleprect_SampleCompute(VCO_bleprect_t *vco) {
	...
	if (syncin >= 1e-20f) { /* sync to master */
		...
		if (!k) {
			if (p_at_reset >= b) {
				place_step_dd(vco->_f, j, p_at_reset - b + eof_offset, w,
						-1.0f);
				k = 1;
				x = -0.5f;
			}
	...
	if (syncin >= 1e-20f) { /* sync to master */
		...
		if (!k) {
			...
			if (p_at_reset >= b) {
				...
				place_slope_dd(vco->_f, j, p_at_reset - b + eof_offset, w,
						-1.0f / b1 - 1.0f / b);
				k = 1;
			}
			if (p_at_reset >= 1.0f) {
				p_at_reset -= 1.0f;
				x = -0.5f + p_at_reset / b;
				place_slope_dd(vco->_f, j, p_at_reset + eof_offset, w,
						1.0f / b + 1.0f / b1);
				k = 0;
			}
		} else {
